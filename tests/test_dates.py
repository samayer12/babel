...
class FormatTimedeltaTestCase(unittest.TestCase):
    ...
    def test_format_timedelta_all_times_minutes(self):
        ...
        for times in exhaustive:
            ...
            if sum(times[0:3]) == 0:
                expected = '0 minutes'
            elif sum(times[0:2]) != 0:
                expected = re.sub(r'((^|, )0 [a-z]*(|$))', '',
                                  '{0}, {1}, and {2}'.format(_pluralize(times[0], 'day'), _pluralize(times[1], 'hour'),
                                                              _pluralize(times[2], 'minute')))
            else:
                expected = re.sub(r'((^|, )0 [a-z]*(|$))', '',
                                  '{0}, {1}, {2}'.format(_pluralize(times[0], 'day'), _pluralize(times[1], 'hour'),
                                                              _pluralize(times[2], 'minute')))
            expected = re.sub(r'(^, )', '', expected)

            result = format_timedelta(timedelta(days=times[0], hours=times[1], minutes=times[2], seconds=times[3]),
                                      granularity='minute', threshold=1)
            if expected != result:
